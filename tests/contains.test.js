...
test("contains: groups", () => {
  ...
  expect(contains(GROUPS.STRING, { __type: "regex", value: /a/ })).toBe(true);
...
test("contains: regex", () => {
  expect(contains({ __type: "regex", value: /a/ }, "abc")).toBe(true);
  expect(contains({ __type: "regex", value: /h/ }, "abc")).toBe(false);
  expect(contains({ __type: "regex", value: /h/ }, 1)).toBe(false);
  expect(contains({ __type: "regex", value: /h/ }, GROUPS.STRING)).toBe(false);
});

test("contains: range", () => {
  expect(contains({ __type: "range", value: { start: { pos: 0 } } }, 10)).toBe(
    true
  );
  expect(contains({ __type: "range", value: { start: { pos: 0 } } }, 0)).toBe(
    false
  );
  expect(contains({ __type: "range", value: { start: { pos: 0 } } }, -5)).toBe(
    false
  );
  expect(
  ...
  expect(contains({ __type: "range", value: { end: { pos: 0 } } }, -5)).toBe(
    true
  );
  expect(contains({ __type: "range", value: { end: { pos: 0 } } }, 0)).toBe(
    false
  );
  expect(contains({ __type: "range", value: { end: { pos: 0 } } }, 5)).toBe(
    false
